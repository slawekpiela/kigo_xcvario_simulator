...
class XcvarioConfigTests(unittest.TestCase):
        ...
        config = parse_runtime_config(
            {
                ...
                "primary_device": "sxhawk",
                "home_position": {
                    "latitude_deg": 49.83833,
                    "longitude_deg": 19.00202,
                    "gps_altitude_m": 401.0,
                },
                "control_api": {"bind_host": "127.0.0.1", "port": 8181},
                "xcvario": {"bind_host": "127.0.0.1", "port": 4353, "polar_name": "DG 800B/15"},
                "flarm": {"bind_host": "127.0.0.1", "port": 4354},
                "scheduler": {"tick_hz": 10, "ownship_hz": 2, "traffic_hz": 1},
            }
        )

        self.assertEqual(config.primary_device, "sxhawk")
    ...
    def test_profile_snippet_uses_lx_driver_for_sxhawk_protocol_when_selected(self):
        snippet = build_xcsoar_profile_snippet("192.0.2.10", primary_device="sxhawk")

        self.assertIn('DeviceA="LX"', snippet)
        self.assertIn('PortTCPPort="4353"', snippet)
